...
const ContactSection: React.FC = () => {
  ...
  const [showMap, setShowMap] = useState(false); // State for lazy-loading map
  ...
  const openLocation = () => {
    ...
    if (isIOS) {
      const appleMapsUrl = `maps:?q=Lackea+AB`;
      const googleMapsUrl = `comgooglemaps://?q=Lackea+AB`;

      const userChoice = window.confirm(
        "Open location in Apple Maps or Google Maps?\n\nClick 'OK' for Apple Maps or 'Cancel' for Google Maps."
      );
  ...
  const loadMap = () => {
    setShowMap(true); // Set state to true when user clicks to load map
  };
          ...
          <div className="bg-gray-800/70 backdrop-blur-lg rounded-2xl p-6 md:p-8 shadow-2xl border border-gray-700/50 transform transition-all hover:scale-[1.02]">
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="flex items-center">
                ...
                <a href="mailto:info@lackea.se" className="text-gray-300 hover:text-white transition-colors">
                  {t("email")}
                </a>
              </div>
              <div>
                <div className="flex items-start mb-4">
                  ...
                  <div className="flex flex-col">
                    <a href="tel:+46707799026" className="text-gray-300 hover:text-white transition-colors">
                      {t("directNumber")}
                    </a>
                    <a href="tel:+46866441020" className="text-gray-300 hover:text-white transition-colors">
                      {t("switchboard")}
                    </a>
                  </div>
                </div>
              ...
              <div className="flex items-center space-x-2 mt-4">
                ...
                <a href="https://wa.me/46707799026" className="text-gray-300 hover:text-white transition-colors">
                  WhatsApp
                </a>
              </div>

              {/* Lazy-Loaded Google Map */}
              <div className="mt-6 rounded-lg overflow-hidden shadow-lg h-64 md:h-80 relative">
                {!showMap ? (
                  <div
                    className="absolute inset-0 bg-gray-900 flex items-center justify-center cursor-pointer"
                    onClick={loadMap}
                  >
                    <p className="text-gray-400 text-lg">Click to load map</p>
                  </div>
                ) : (
                  <iframe
