            ...
            <div className="bg-gray-800 rounded-xl p-6 md:p-8 shadow-2xl order-1 md:order-2">
              <div className="grid grid-cols-2 gap-4 md:gap-6">
                <div className="text-center">
                  ...
                  <h3 className="text-2xl md:text-3xl font-bold text-white">{t('location.title')}</h3>
                  <p className="text-xs md:text-base text-gray-400">{t('location.subtitle')}</p>
                </div>
                <div className="text-center">
                  <Car className="w-8 h-8 md:w-12 md:h-12 text-yellow-500 mx-auto mb-2 md:mb-4" />
                  <h3 className="text-2xl md:text-3xl font-bold text-white">{t('capacity.title')}</h3>
                  <p className="text-xs md:text-base text-gray-400">{t('capacity.subtitle')}</p>
                </div>
              </div>
              <div className="mt-6 md:mt-8 text-center">
                <div className="flex items-center justify-center mb-3 md:mb-4">
                  <Users className="text-purple-500 mr-2 md:mr-3 w-5 h-5 md:w-6 md:h-6" />
                  <span className="text-xs md:text-base text-gray-300">{t('customers.title')}</span>
                </div>
